...
export const CreateBeamAdminForm = (props: any) => {
  ...
  const onSubmitValues = (values: any) => {
    ...
    setLoading(true);
    UserSevice.saveAdmin(payload).then((x: any) => {
      if (x?.code == 200 && x?.message == 'OK') {
        Notification.success({
          content: 'Thêm mới quản trị viên thành công',
          theme: 'light',
        });
        setLoading(false);
        ...
        Notification.error({
          title: 'Error',
          content: 'Thêm mới quản trị viên thất bại!',
          duration: 3,
          theme: 'light',
        });
        setLoading(false);
