...
export const FeePolicyForm = (props: any) => {
  const { feePolicyId, isNew = true, onCancel, onSave } = props;
  ...
  useEffect(() => {
    if (!isLoading && !isNew) {
      ...
      if (data?.feeType == 1) {
        resetData = {
          ...resetData,
          startDate: DateTimeHelper.convertTimeZone(
            data?.startTime,
            COMMON_FORMAT.EMPTY_FORMAT
          ),
          endDate: DateTimeHelper.convertTimeZone(
            data?.endTime,
            COMMON_FORMAT.EMPTY_FORMAT
          ),
        };
  ...
  const onSubmitForm = (values: any) => {
    ...
    requestObject = {
      ...requestObject,
      startTime: DateTimeHelper.fomartDateRangeSubmit(values.dateRange[0]),
      endTime: DateTimeHelper.fomartDateRangeSubmit(values.dateRange[1]),
    };
    ...
    FeePolicyService.saveOrUpdateFeePolicy(requestObject).then((x: any) => {
      if (x) {
        ...
        Notification.success({
          content: `${isNew ? 'Tạo mới' : 'Cập nhật'} chính sách phí thất bại!`,
          theme: 'light',
        });
        setLoading(false);
      }
  ...
  const companyRowSelection = {
    onChange: (selectedRowKeys: any, selectedRows: any) => {
      setSelectedCompanies(selectedRowKeys);
    },
  };
      ...
      {currentStep == 2 && (
        <>
          <CompanyList
            ...
            showSelection={true}
          />
          {/* <FormActionButton submitButtonText="Tiếp theo" /> */}
        </>
      )}
    </>
  );
