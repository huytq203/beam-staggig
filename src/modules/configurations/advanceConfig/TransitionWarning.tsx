...
const TransitionWarning = () => {
  ...
  const { data, isLoading, refetch } = useQuery(['transition-warning'], () =>
    ConfigurationService.getAdvanceConfigurationByType(1)
  );
  const columns = [
    {
      title: 'Tên cảnh báo',
      ...
      render: (e: any) => {
        ...
        switch (e) {
          case 'CONTINUOUS_WITHDRAWAL':
            label = 'Giao dịch rút tiền liên tục';
            break;
          case 'BIG_WITHDRAW':
            label = 'Giao dịch rút tiền lớn';
            break;
        }
        return <p>{label}</p>;
      },
