...
const ImportBonusInvitedForm = (props: ImportWorkDayEmployeeFormProps) => {
  ...
  const {
    ...
    () => FriendInvatationService.getImportBonusInvitedTemplate(),
    {
  ...
  const onUpload = async (data: any) => {
    setLoading(true);
    return await FriendInvatationService.importBonusInvited(data).then(
      (x: any) => {
        if (x?.data?.code == 200) {
          setLoading(false);
          // refetchLog();
  ...
  return (
    <ContentWrapper pageTitle="Tải lên danh sách chi thưởng người được giới thiệu">
      <div className="flex flex-col gap-4 mt-4">
        ...
        <div className="flex flex-col gap-4">
          <div>Tải lên danh sách chi thưởng người được giới thiệu</div>
          <Upload
            ref={uploadRef}
            limit={1}
            ...
            onSuccess={(e: any) => {
              Notification.success({
                content: `Tải lên danh sách người được giới thiệu đạt thưởng thành công!`,
                theme: 'light',
              });

              setListUploaded([...listUploaded, e]);
              router.push('/invite-friends/bonus/bonus-presentee');
              //   Modal.success({
        ...
        <div>
          ...
          <ul className="list-none">
            <li>
              1. Tải file mẫu danh sách để nhập thông tin người được giới thiệu
              đạt tiêu chuẩn nhận thưởng
            </li>
            <li>
              2. Thực hiện điền danh sách người được giới thiệu vào file mẫu
            </li>
...
export default ImportBonusInvitedForm;
