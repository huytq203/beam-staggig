...
export const ExportVoucherDiscountListButton = (props: any) => {
  const { filterExport } = props;

  const onClickExportData = async () => {
    const downloadData: any = await LuckyBoxService.exportBonusVoucherDiscount(
      filterExport
    );
    return downloadData;
  };
    ...
    <>
      ...
      >
        <DownloadLink
          ...
          filename={`Danhsachchithuong_Vouchergiamgia_${DateTimeHelper.formatDateTime(
            new Date()
          )}.xlsx`}
