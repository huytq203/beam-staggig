...
const ImportCash = (props: ImportBonusPhoneCardProps) => {
  ...
  const onUpload = async (data: any) => {
    setLoading(true);
    return await LuckyBoxService.importCashReward(data).then((x: any) => {
      if (x?.data?.code == 200) {
  ...
  return (
    <div className="flex flex-col gap-4 mt-4">
      <div className="flex flex-col gap-4">
        <div>Tải lên danh sách chi thưởng tiền mặt</div>
        <Upload
          ref={uploadRef}
          limit={1}
          maxSize={10240}
          uploadTrigger="custom"
          action={`${NEXT_PUBLIC_API_CORE}/lucky-box/rewards/imports/cash-reward`}
          onFileChange={() => {
            setHasFile(true);
          }}
          dragIcon={<IconBolt />}
        ...
        <div className="flex">
          ...
          <div>
            <Button onClick={() => router.push('/lucky-box/bonus/cash')}>
              Quay lại
            </Button>
          </div>
        </div>
      ...
      <div>
        ...
        <ul className="list-none">
          <li>1. Xuất danh sách được chi thưởng tiền mặt</li>
