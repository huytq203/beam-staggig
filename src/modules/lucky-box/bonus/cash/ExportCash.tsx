...
export const ExportCashListButton = (props: any) => {
  const { filterExport } = props;

  const onClickExportData = async () => {
    const downloadData: any = await LuckyBoxService.exportBonusCash(
      filterExport
    );
    return downloadData;
  };
    ...
    <>
      ...
      >
        <DownloadLink
          ...
          filename={`Danhsachchithuong_Tienmat_${DateTimeHelper.formatDateTime(
            new Date()
          )}.xlsx`}
