...
export const ExportPhoneCardListButton = (props: any) => {
  const { filterExport } = props;

  const onClickExportData = async () => {
    const downloadData: any = await LuckyBoxService.exportBonusPhoneCard(
      filterExport
    );
    return downloadData;
  };
    ...
    <>
      ...
      >
        <DownloadLink
          ...
          filename={`Danhsachchithuong_Thedienthoai_${DateTimeHelper.formatDateTime(
            new Date()
          )}.xlsx`}
