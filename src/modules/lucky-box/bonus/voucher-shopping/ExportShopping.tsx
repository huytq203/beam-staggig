...
export const ExportVoucherShoppingListButton = (props: any) => {
  const { filterExport } = props;

  const onClickExportData = async () => {
    const downloadData: any = await LuckyBoxService.exportBonusShoppingCard(
      filterExport
    );
    return downloadData;
  };
    ...
    <>
      ...
      >
        <DownloadLink
          ...
          filename={`Danhsachchithuong_Vouchermuasam_${DateTimeHelper.formatDateTime(
            new Date()
          )}.xlsx`}
