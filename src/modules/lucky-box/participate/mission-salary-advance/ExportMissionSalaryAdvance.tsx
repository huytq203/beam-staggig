...
export const ExportMissionSalaryAdvanceButton = (props: any) => {
  const { filterExport } = props;

  const onClickExportData = async () => {
    const downloadData: any = await LuckyBoxService.exportUserTransaction(
      filterExport
    );
    return downloadData;
  };
    ...
    <>
      ...
      >
        <DownloadLink
          ...
          filename={`Danhsachthamgia_NhiemvuUngluong_${DateTimeHelper.formatDateTime(
            new Date()
          )}.xlsx`}
