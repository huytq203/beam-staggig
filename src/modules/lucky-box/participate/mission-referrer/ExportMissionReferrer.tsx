...
export const ExportMissionReferrer = (props: any) => {
  const { filterExport } = props;

  const onClickExportData = async () => {
    const downloadData: any = await LuckyBoxService.exportFriendInvitation(
      filterExport
    );
    return downloadData;
  };
    ...
    <>
      ...
      >
        <DownloadLink
          ...
          filename={`Danhsachthamgia_NhiemvuGTBB_${DateTimeHelper.formatDateTime(
            new Date()
          )}.xlsx`}
