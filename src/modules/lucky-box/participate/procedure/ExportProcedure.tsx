...
export const ExportProcedureButton = (props: any) => {
  const { filterExport } = props;

  const onClickExportData = async () => {
    const downloadData: any = await LuckyBoxService.exportProcedure(
      filterExport
    );
    return downloadData;
  };
    ...
    <>
      ...
      >
        <DownloadLink
          ...
          filename={`Danhsachthamgia_Hoantatthutuc_${DateTimeHelper.formatDateTime(
            new Date()
          )}.xlsx`}
